...
dados_deputados_federais_eleitos = function(con, output, input) {
  ufs = input$selectUfsDeputadosFederaisEleitos
  
  dados = deputados_federais_eleitos(con, ufs)
  
  if (length(dados) == 0) {
    showNotification("Dados de deputados federais não encontrados para esse filtro", type="warning")
    return()
  }
  ...
  plot = ggplotly(plot)
  output$grafico_deputados_federais_eleitos = renderPlotly({plot})
}