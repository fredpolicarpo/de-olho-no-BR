...
dados_senadores_eleitos = function(con, output, input) {
  ...
  dados = senadores_eleitos(con, ufs)
  
  if (length(dados) == 0) {
    showNotification("Dados de senadores não encontrados para esse filtro", type="warning")
    return()
  }
  ...
  plot = ggplotly(plot)
  output$grafico_senadores_eleitos = renderPlotly({plot})
}